...
class SIPEngine:
    ...
    def load_config(self, config_path: str = "config/sip_config.json") -> bool:
        ...
        try:
            ...
            # Check if it's multi-account format
            if "mode" in self.config and self.config["mode"] == "multi_account":
                logger.info("Detected multi-account SIP configuration")
                # Multi-account format - validate structure
                if "lines" not in self.config or not isinstance(self.config["lines"], list):
                    logger.error("Multi-account config missing 'lines' array")
                    return False
                
                if "sip_server" not in self.config:
                    logger.error("Multi-account config missing 'sip_server'")
                    return False
                
                # Set defaults for multi-account
                self.config.setdefault("sip_port", 5060)
                self.config.setdefault("transport", "UDP")
                self.config.setdefault("num_lines", len(self.config["lines"]))
                
                logger.info(f"Loaded multi-account SIP config from {config_path}")
                logger.info(f"SIP Server: {self.config['sip_server']}:{self.config['sip_port']}")
                logger.info(f"Number of lines: {self.config['num_lines']}")
                logger.info(f"Transport: {self.config['transport']}")
                return True
            
            # Single-account format - legacy validation
